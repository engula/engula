...
mod tests {
    ...
    async fn simple() -> std::result::Result<(), Box<dyn std::error::Error>> {
        ...
        let journal = file::Journal::create(tmp.path()).await?;
    ...
    #[tokio::test]
    async fn many_write_read() -> std::result::Result<(), Box<dyn std::error::Error>> {
        let tmp = tempfile::tempdir()?;
        let journal = file::Journal::create(tmp.path()).await?;
        let stream = journal.create_stream("s").await?;
        let init_ts = 31340128116183;
        ...
        for i in 0..1000 {
            ...
            let mut events = stream.read_events(ts.into()).await;
            let got = events.try_next().await?.unwrap();
            assert_eq!(got.len(), (1000 - i) as usize);
        }
    ...
    #[tokio::test]
    async fn big_data() -> std::result::Result<(), Box<dyn std::error::Error>> {
        let tmp = tempfile::tempdir()?;
