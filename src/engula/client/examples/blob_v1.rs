...
async fn main() -> Result<()> {
    ...
    println!("a.len = {:?}", len);
    let world: Vec<u8> = co.select("a", Blob::range(5..)).await?;
    println!("a.range(5..) = {:?}", world);
    let world: Vec<u8> = co.mutate("a", Blob::pop_back(5)).await?;
    println!("a.pop_back(5) = {:?}", world);

    let mut txn = co.begin();
    txn.mutate("a", Blob::push_back("hello"));
    txn.mutate("b", Blob::push_back("world"));
    txn.commit().await?;
