...
#[async_trait]
impl<K: Kernel> crate::Engine for Engine<K> {
...
impl<K: Kernel> Engine<K> {
    pub async fn start(kernel: K) -> Result<Self> {
        ...
        let current = EngineVersion::new(bucket.clone(), version).await?;
        let engine = Self {
            kernel,
            stream,
            bucket,
...
impl<K: Kernel> EngineVersion<K> {
    async fn new(bucket: K::Bucket, version: Arc<Version>) -> Result<Self> {
        ...
        Ok(Self {
            ...
            last_timestamp,
            last_object_number,
            mem: Arc::new(Memtable::new(last_timestamp)),
            imm: VecDeque::new(),
            tables,
        })
    }
