...
use super::{heartbeat::DeltaStats, metrics, RootShared};
...
impl<T: AllocSource> Allocator<T> {
    pub fn new(alloc_source: Arc<T>, delta_stats: Arc<DeltaStats>, config: RootConfig) -> Self {
        Self {
            alloc_source,
            config,
            delta_stats,
    ...
    pub async fn compute_replica_action(&self) -> Result<Vec<ReplicaAction>> {
        ...
        let actions =
            ReplicaCountPolicy::with(self.alloc_source.to_owned(), self.delta_stats.to_owned())
                .compute_balance()?;
        if !actions.is_empty() {
            return Ok(actions);
        }
